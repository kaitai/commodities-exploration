...
sbv_xts <- xts(sbv24_daily_nearby_historical_data_07_10_2023, order.by = as.POSIXct(sbv24_daily_nearby_historical_data_07_10_2023$Time))
```
...
```{r rip off bsts }
Y <- as.numeric(sbv_xts$Open)
y <- log10(Y)
# let's just try it, even if it's not so good. 

### Run the bsts model
ss <- AddLocalLinearTrend(list(), y)
ss <- AddSeasonal(ss, y, nseasons = 365)
bsts.model <- bsts(y, state.specification = ss, niter = 500, ping=0, seed=42)

### Get a suggested number of burn-ins
burn <- SuggestBurn(0.1, bsts.model)

### Predict
p <- predict.bsts(bsts.model, horizon = 365, burn = burn, quantiles = c(.025, .975))
```